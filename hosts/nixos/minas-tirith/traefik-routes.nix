...
let
  ...
  # ---------------------------------------------------------------------------
  # host      — the public hostname, already covered by the *.saldivar.io wildcard
  #             on the `https` entrypoint, so no certResolver is needed per-route.
  # namespace — k8s namespace.
  # port      — the Service port (not the container port).
  routes = {
    audiobookshelf = {
      host = "listen.saldivar.io";
      namespace = "books";
      port = 80;
    };
  };
