...
if ! findmnt -no FSTYPE /storage2 | grep -qx zfs; then
...
if ! zfs list -H -o name storage2/backup >/dev/null 2>&1; then
...
if [ "$(zfs get -H -o value mounted storage2/backup)" != "yes" ]; then
  echo "ABORT: storage2/backup exists but is NOT MOUNTED; snapshots would be empty" >&2
  exit 1
fi
if [ "$(zfs get -H -o value mountpoint storage2/backup)" != "/storage2/backup" ]; then
...
if k3s crictl ps -q >/dev/null 2>&1; then
  for id in $(k3s crictl ps -o json 2>/dev/null \
              | grep -oE '"id": "[a-f0-9]{12,}"' \
              | sed 's/.*"\([a-f0-9]*\)"$/\1/' || true); do
    ...
    img=$(k3s crictl inspect "$id" 2>/dev/null \
          | grep -oE '"(image|imageRef)": "[^"]*"' \
          | grep -oiE '(postgres|pgvector|pgvecto)[^"]*' | head -1 || true)
    [ -n "$img" ] || continue
    ...
    kns=$(k3s crictl inspect "$id" 2>/dev/null \
          | grep -oE '"io.kubernetes.pod.namespace": "[^"]+"' | head -1 \
          | sed 's/.*"\([^"]*\)"$/\1/' || true)
    kctr=$(k3s crictl inspect "$id" 2>/dev/null \
           | grep -oE '"name": "[^"]+"' | head -1 \
           | sed 's/.*"\([^"]*\)"$/\1/' || true)
    ...
    if [ "$kns" = authentik ] && [ "$kctr" = authentik-postgresql ]; then
      ...
      if k3s crictl exec "$id" pg_dumpall -U "$u" 2>/dev/null \
         | gzip -c \
         | age -r "$authentik_age_admin" -r "$authentik_age_pelargir" \
           > "$out.tmp"; then
        if [ "$(stat -c %s "$out.tmp")" -gt 1024 ] \
    ...
    if k3s crictl exec "$id" pg_dumpall -U "$u" 2>/dev/null \
       | gzip -c > "$dumpdir/$nm.sql.gz.tmp"; then
      if [ "$(stat -c %s "$dumpdir/$nm.sql.gz.tmp")" -gt 1024 ]; then
...
for entry in "books|readmeabook|postgres|" "media|tracearr|tracearr|fc" ; do
  ...
  did=$(k3s crictl ps --namespace "$dns" --name "$dctr" -q 2>/dev/null | head -1 || true)
  if [ -z "$did" ]; then
  ...
  if [ "$dmode" = "fc" ]; then
    out="$dumpdir/$nm.dump"
    dumpcmd_ok=0
    k3s crictl exec "$did" pg_dump -U "$duser" -Fc -d "$duser" > "$out.tmp" 2>/dev/null && dumpcmd_ok=1
    if [ "$dumpcmd_ok" = "1" ] && [ "$(stat -c %s "$out.tmp")" -gt 1024 ]; then
      mv "$out.tmp" "$out"
  ...
  if k3s crictl exec "$did" pg_dumpall -U "$duser" 2>/dev/null \
     | gzip -c > "$dumpdir/$nm.sql.gz.tmp"; then
...
if [ -e /var/lib/healthcheck-ping/authentik.expected ]; then
  ...
  if [ ! -f "$auth_data_out" ]; then
    ...
    auth_data_age=$(( $(date -u +%s) - $(stat -c %Y "$auth_data_out") ))
...
for qn in jellyfin; do
  ...
  if grep -q '^FAILED' "$qf" 2>/dev/null; then
  ...
  if grep -q '^staged ' "$qf" 2>/dev/null; then
    sbytes=$(grep -oE '^staged .*bytes=[0-9]+' "$qf" 2>/dev/null \
             | grep -oE 'bytes=[0-9]+' | cut -d= -f2 || true)
    if [ -z "$sbytes" ]; then
  ...
  qtbl=$(grep -oE '^success .*tables=[0-9]+' "$qf" 2>/dev/null \
         | grep -oE 'tables=[0-9]+' | cut -d= -f2 || true)
  qbytes=$(grep -oE '^success .*bytes=[0-9]+' "$qf" 2>/dev/null \
           | grep -oE 'bytes=[0-9]+' | cut -d= -f2 || true)
...
for entry in \
  ...
  "/usr/local/etc/shelfmark/config/users.db|" \
...
; do
  ...
  lockdir=""
  case "$db" in
    /storage2/backup/staging/*/*/*) lockdir=$(dirname "$(dirname "$db")") ;;
  esac
  if [ ! -f "$db" ]; then
  ...
  if [ -n "$stopc" ] && docker ps --format '{{.Names}}' 2>/dev/null \
    ...
    if docker stop -t 30 "$stopc" >/dev/null 2>&1; then
      ...
      trap 'if [ -n "$restart_needed" ] && \
               ! docker start "$restart_needed" >/dev/null 2>&1; then
  ...
  stage="$dumpdir/.stage"
  rm -rf "$stage"
  install -d -m 0700 -o "$owner" -g "$group" "$stage"
  if setpriv --reuid="$owner" --regid="$group" --clear-groups \
    ...
    if [ "$vmode" = "nointegrity" ]; then
      ...
      ok=$(sqlite3 "$stage/$n.tmp" "PRAGMA integrity_check;" 2>/dev/null || echo bad)
    fi
    tbls=$(sqlite3 "$stage/$n.tmp" \
    ...
    dsz=$(stat -c %s "$stage/$n.tmp" 2>/dev/null || echo 0)
  ...
  if [ -n "$restart_needed" ]; then
    if docker start "$restart_needed" >/dev/null 2>&1; then
...
zfs snapshot "storage2/backup@daily-$ts"
...
if [ "$(date -u +%u)" = "7" ]; then
  zfs snapshot "storage2/backup@weekly-$ts"
fi
...
prune() {
  ...
  snaps=$(zfs list -H -o name -t snapshot -s creation -r storage2/backup 2>/dev/null \
          | grep "@$prefix-" || true)
  [ -z "$snaps" ] && return 0
  total=$(printf '%s\n' "$snaps" | grep -c . || true)
