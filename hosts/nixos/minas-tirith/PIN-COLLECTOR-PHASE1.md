...
without printing them. Pelargir renders Kubernetes Secret manifests in tmpfs and applies
them through `k3s-apply-secrets`. The worker node cannot decrypt the SOPS document.
...
containerd metadata. The remaining key is
...
missing registry gate, mutable image, or wrong repository. Rebuild
pelargir first so namespace, Secrets, PVCs, workloads, and migration land before minas
publishes the Traefik route.
