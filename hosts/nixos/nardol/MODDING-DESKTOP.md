...
| --- | --- | --- |
| `/home/retro/Games/<launcher>` | A launcher's game library, prefixes, manifests, and Workshop content | read/write |
...
Add libraries as siblings such as `Games/Steam`, `Games/GOG`, or
`Games/Lutris`. Host paths are deployment inputs and can differ on every
machine. The image does not depend on their names or locations.

For Nardol, Steam's `steamapps` directory is mounted at `Games/Steam`. That one
tree includes installed games (`common`), Proton prefixes (`compatdata`),
Workshop content, manifests, and Steam's library metadata. Elden Ring and most
Windows-game modding therefore need no additional runtime-state mount.
...
mounts = [
  "/host/games/steamapps:/home/retro/Games/Steam:rw",
