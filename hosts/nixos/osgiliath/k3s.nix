...
}:
{
  ...
  fleet.k3sNode = {
    ...
    # K3s ServiceLB otherwise places its 80/443 hostPort DaemonSet on every
    # eligible node. A dedicated taint keeps Pelargir's ingress listener off
    # this node; only the explicitly tolerating Osgiliath workloads run here.
