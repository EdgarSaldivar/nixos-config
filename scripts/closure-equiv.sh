...
for host in nardol minas-tirith osgiliath pelargir; do
  ...
  " 2>/dev/null | grep -oE '[a-z0-9]{32}' || echo 'EVAL-FAILED'
  echo
done

printf '%-14s ' "dol-amroth"
# darwin does not go through mkNixos, so it carries no revision stamp.
nix eval --raw "$TREE#darwinConfigurations.dol-amroth.config.system.build.toplevel.drvPath" 2>/dev/null \
  | grep -oE '[a-z0-9]{32}' || echo 'EVAL-FAILED'
echo
