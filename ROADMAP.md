...
- **Revisit the Seerr memory limit when v3.5.0 ships.** The 3Gi limit in
  ...
  from another fleet's reported figures rather than from any measurement of this
  one. Measure it here across a full scheduled-scan cycle and reset both the
  request and the limit from that. Do not lower either from an idle RSS reading:
  the failure is a V8 heap abort rather than a cgroup OOM kill, so `OOMKilled` is
  false and the evidence is a `FATAL ERROR: ... heap limit` line on stderr that
  never reaches the application log. PR #3367 is expected in v3.5.0; re-evaluate
  once a release actually contains it.
